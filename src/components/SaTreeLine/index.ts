...
export default defineComponent({
    ...
    setup(_: any, context: { slots: any }) {
        ...
        const getSlotValue = (slot: unknown, scopedData: any, defaultNode = null) => {
            if (isFunction(slot)) {
    ...
    render() {
        ...
        let currentNode = this.node as any
        while (currentNode) {
            let parentNode = currentNode.parent as any
            ...
            if (parentNode) {
                ...
                const index = (parentNode.children || parentNode.childNodes).findIndex(
                    (item: { key: any; id: any }) => (item.key || item.id) === (currentNode.key || currentNode.id)
                )
                lastnodeArr.unshift(index === (parentNode.children || parentNode.childNodes).length - 1)
            }
            currentNode = parentNode as TreeNode
        }
        const lineNodes = []
        for (let i = 1; i < this.node.level; i++) {
            lineNodes.push(
                h('span', {
                    ...
                    style: { left: this.indent + (i - 1) * 18 + 'px' },
                })
            )
        }
        ...
        return h(
            ...
            [labelNodes].concat(lineNodes).concat([
                h('span', {
                    class: {
                        'element-tree-node-line-hor': this.node.level - 1 !== 0,
                    },
                    style: {
                        width: (this.node.isLeaf ? 24 : 8) + 'px',
                        left: (this.node.level - 1) * this.indent + (this.node.level - 2) * 6 + 'px',
                    },
                }),
            ])
        )
